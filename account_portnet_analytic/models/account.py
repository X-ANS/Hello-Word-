                ...
                if obj_line.key_activity_id:
                    for line in obj_line.key_activity_id.key_activity_lines:
                        if line.prestation_id:
                            ...
                            key_val_ids = self.env['analytic.key.value'].read_group(
                                [('analytic_key_id', '=', line.key_id.id)]
                                , ['prestation_id', 'value'],
                                ['prestation_id'])
                            key_val_dict = {}
    ...
    def post(self):
        for rec in self:
            for line in rec.line_id:
                if line.account_id.code.startswith('6') and (
                        (not line.activity_id and not line.key_activity_id) or not line.analytic_account_id):
